 ...
 *  professional). Skippable — KYC is already complete after the Aadhaar step. Real document
 *  upload + company-email OTP verification + `PATCH /profile/tenant-profile/:id`. */
...
export default function KycOccupation() {
  ...
  const docs = occupation === 'STUDENT' ? [collegeIdCard, admissionLetter] : [employeeIdCard, employmentLetter];
  const hasAnyDoc = docs.some(Boolean);

  const submit = async () => {
    if (!hasAnyDoc && !enrollmentNo.trim() && !companyEmail.trim()) {
      finishLater();
      return;
    }
    if (occupation === 'WORKING_PROFESSIONAL' && companyEmail.trim() && !companyEmailVerified) {
      alert('Verify your company email', 'Please verify your company email before submitting, or clear it to skip.');
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <View style={{ flex: 1, paddingTop: insets.top + spacing.sm }}>
      ...
      <View style={{ paddingHorizontal: spacing.xl, paddingBottom: insets.bottom + spacing.base, paddingTop: spacing.sm, gap: spacing.sm }}>
        <Button label="Submit & verify" loadingLabel="Saving…" loading={saving} onPress={submit} full size="lg" />
        <Button label="Skip for now" variant="ghost" onPress={finishLater} full disabled={saving} />
      </View>
    </View>
  );
}
