...
export default function ComingSoon() {
  ...
  const { title, subtitle, icon } = useLocalSearchParams<{ title?: string; subtitle?: string; icon?: string }>();
  ...
  const glyph = (icon as keyof typeof Ionicons.glyphMap) ?? 'rocket-outline';
  ...
  return (
    <View style={{ flex: 1, backgroundColor: palette.bg, paddingTop: insets.top + spacing.sm }}>
      ...
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: spacing.xl, paddingBottom: insets.bottom + spacing['4xl'] }}>
        ...
        >
          <Ionicons name={glyph} size={52} color={palette.white} />
        </LinearGradient>
