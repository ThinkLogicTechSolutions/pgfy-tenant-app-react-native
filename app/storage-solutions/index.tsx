...
import type { StorageSolutionEnquiryResponse, StorageSolutionEnquiryStatus } from '@/lib/api/types';
...
const BANNER_SLIDES = [
  { icon: 'shield-checkmark-outline', text: 'Secure Warehousing' },
  { icon: 'videocam-outline', text: '24/7 Surveillance' },
  { icon: 'calendar-outline', text: 'Flexible Durations' },
];
...
export default function StorageSolutionsHistory() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const [activeSlide, setActiveSlide] = useState(0);
  const carouselRef = useRef<FlatList>(null);

  const [status, setStatus] = useState<StorageSolutionEnquiryStatus | 'ALL'>('ALL');
  const [dateFilter, setDateFilter] = useState('ALL');
  const [filterOpen, setFilterOpen] = useState(false);
  const [draftStatus, setDraftStatus] = useState<StorageSolutionEnquiryStatus | 'ALL'>('ALL');
  const [draftDateFilter, setDraftDateFilter] = useState('ALL');

  const [enquiries, setEnquiries] = useState<StorageSolutionEnquiryResponse[]>([]);
  ...
  const loadPage = (skip: number) => {
    ...
    storageSolutionApi.listStorageSolutionEnquiries({limit: PAGE_SIZE, skip, status, from_date, to_date})
      .then((page) => {
  ...
  return (
    <View style={{ flex: 1, paddingTop: insets.top + spacing.xs, backgroundColor: palette.background }}>
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <AnimatedListItem index={index}>
              <PressableScale
                onPress={() => router.push({ pathname: '/storage-solutions/[id]', params: { id: String(item.id) } })}
                scaleTo={0.99}
              ...
              >
                <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: spacing.sm }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm, flex: 1 }}>
                    <View style={{ width: 44, height: 44, borderRadius: 22, backgroundColor: palette.coralTint, alignItems: 'center', justifyContent: 'center' }}>
                      <MaterialCommunityIcons name="package-variant-closed" size={24} color={palette.coral} />
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text variant="bodyMd" weight="700">STR-SOL-{String(item.id).padStart(3, '0')}</Text>
                      ...
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 2 }}>
                        <Ionicons name="location-outline" size={12} color={palette.inkSecondary} />
                        <Text variant="caption" color={palette.inkSecondary} numberOfLines={1}>
          ...
          ListEmptyComponent={
            <EmptyState
              ...
              message={error ?? (filterActive ? 'Try a different filter.' : "You haven't made any Storage Solutions enquiries yet.")}
            />
          }
      ...
      <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, paddingHorizontal: spacing.base, paddingTop: spacing.md, paddingBottom: insets.bottom + spacing.md, backgroundColor: palette.surface, borderTopWidth: 1, borderTopColor: palette.border }}>
        <Button label="Create new enquiry" onPress={() => router.push('/storage-solutions/create-enquiry')} full size="lg" />
      </View>
          ...
          <View>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm }}>
              {STATUS_FILTERS.map((f) => (
                <Chip key={f.key} label={f.label} active={draftStatus === f.key as StorageSolutionEnquiryStatus | 'ALL'} onPress={() => setDraftStatus(f.key as StorageSolutionEnquiryStatus | 'ALL')} />
              ))}
            </View>
          </View>
